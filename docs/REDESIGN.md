...
| ECHO-01 | **哈囉式 (Haro-style) round companion robot — the pilot insisted, this overrides the earlier "no Haro silhouette" rule.** A glossy GREEN sphere (~basketball size in the cockpit), two hinged flap "ears" on top that swing open/closed as it talks and emotes, two round luminous eyes on the front, a thin mouth slot, and small stubby arms + legs that pop out of hatches when it is excited and retract when idle. It BOUNCES on the spot instead of hovering, and can roll. It sits on a padded cradle on the cockpit's upper-right console. Keep the name ECHO-01 and the callout text; the look is the classic round-green-ball buddy. Reacts to `aiStatus` (IDLE slow bounce, ears closed · LISTENING ears open, eyes brighten · THINKING ears flutter, eyes blink · SPEAKING ears flap in rhythm, mouth slot lights), to `fx:playerHit` (tumbles, eyes flash red), to a kill (jumps + spins, arms pop out), to phase BOSS_INTRO (ears clamp shut, eyes narrow), to low `player.hp` (fast nervous bounce). |
